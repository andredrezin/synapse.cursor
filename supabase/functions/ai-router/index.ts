...
const containsTransferKeyword = (message: string, keywords: string[] | null): boolean => {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!body.task || !body.workspace_id) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: task, workspace_id' }),
    ...
    if (settingsError && settingsError.code !== 'PGRST116') {
      log('WARN', `[${requestId}] Error fetching AI settings`, { error: settingsError.message });
    }
    ...
    if (!skipValidationTasks.includes(body.task)) {
      ...
      if (messageContent && settings && containsTransferKeyword(messageContent, settings.transfer_keywords)) {
