...
const analyzeWithLovableAI = async (
  ...
  const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { 
          role: 'system', 
          content: 'Você é um analisador de conversas de vendas. Extraia conhecimento útil das respostas dos vendedores. Responda sempre em JSON válido.' 
        },
  ...
  if (!response.ok) {
    ...
    log('error', 'AI Gateway error', { error });
    return null;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!lovableApiKey) {
      log('warn', `[${requestId}] LOVABLE_API_KEY not configured`);
      return new Response(JSON.stringify({ learned: false, reason: 'no_api_key' }), {
    ...
    const { workspace_id, conversation_id, message_id, message_content, sender_type, seller_profile_id, previous_messages } = body;

    log('info', `[${requestId}] Processing learning request`, { workspace_id, sender_type });
    ...
    if (!trainingStatus || trainingStatus.status === 'active') {
      ...
      return new Response(JSON.stringify({ learned: false, reason: 'training_complete' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (sender_type === 'agent' && seller_profile_id) {
      ...
      const conversationContext = previous_messages
        .slice(-5)
        .map(m => `${m.sender_type === 'lead' ? 'Cliente' : 'Vendedor'}: ${m.content}`)
        .join('\n');
...
Se não houver nada útil para aprender, retorne {"has_learning": false}`;
      ...
      log('info', `[${requestId}] Calling Lovable AI for analysis`);
      const analysisResult = await analyzeWithLovableAI(lovableApiKey, analysisPrompt);

      if (analysisResult?.has_learning && (analysisResult.confidence as number) > 0.6) {
        ...
        const { data: existing } = await supabase
          ...
          .ilike('answer', `%${(analysisResult.answer as string).substring(0, 50)}%`)
          .limit(1);
        ...
        if (existing && existing.length > 0) {
          ...
          await supabase
            ...
            .update({ 
              ...
              effectiveness_score: Math.min(100, (existing[0].occurrence_count + 1) * 10),
              updated_at: new Date().toISOString()
            })
            .eq('id', existing[0].id);

          log('info', `[${requestId}] Updated existing learned content`, { id: existing[0].id });
        } else {
          ...
          if (insertError) {
            log('error', `[${requestId}] Failed to insert learned content`, { error: insertError.message });
          } else {
            log('info', `[${requestId}] Inserted new learned content`, { type: analysisResult.content_type });
            ...
            if (updateField) {
              const currentValue = trainingStatus[updateField as keyof typeof trainingStatus] as number || 0;
              await supabase
                .from('ai_training_status')
                .update({ 
        ...
        if (updatedStatus) {
          const daysElapsed = Math.floor((Date.now() - new Date(updatedStatus.started_at).getTime()) / (1000 * 60 * 60 * 24));
          const meetsTime = daysElapsed >= updatedStatus.min_days_required;
          const meetsMessages = updatedStatus.messages_analyzed >= updatedStatus.min_messages_required;

          if (meetsTime && meetsMessages && updatedStatus.status === 'learning') {
            await supabase
              .from('ai_training_status')
              .update({ 
                status: 'ready',
                ready_at: new Date().toISOString(),
                confidence_score: Math.min(100, (updatedStatus.faqs_detected * 5 + updatedStatus.seller_patterns_learned * 3)),
                updated_at: new Date().toISOString()
              })
              .eq('id', updatedStatus.id);

            log('info', `[${requestId}] AI training marked as ready!`, { workspace_id });
          }
        }
    ...
    return new Response(JSON.stringify({ learned: false, reason: 'no_learning_detected' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });

  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    log('error', `[${requestId}] Unhandled error in ai-learn`, { error: errorMessage });
