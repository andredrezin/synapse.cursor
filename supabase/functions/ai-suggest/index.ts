      ...
      messages: [
        {
          role: "system",
          content:
            "Você é um assistente de vendas expert que ajuda vendedores a responder leads. Responda sempre em JSON válido.",
        },
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: context } = await supabase
      ...
      .select(
        "sentiment, intent, topics, summary, mentioned_products, mentioned_services"
      )
...
- Resumo: ${context.summary}`;
    ...
    log("INFO", `[${requestId}] Calling OpenAI for suggestions`);
    const suggestions = await suggestWithOpenAI(openAiApiKey, fullContext);
