...
const selectAgent = (message: string, task: string): AgentConfig => {
    const analyticsKeywords = [
        'métrica', 'relatório', 'análise', 'roi', 'conversão',
        'cálculo', 'estatística', 'performance', 'kpi', 'dashboard'
    ];

    const lowerMessage = message.toLowerCase();

    if (analyticsKeywords.some(kw => lowerMessage.includes(kw)) ||
        task === 'analyze' || task === 'qualify') {
...
serve(async (req) => {
    ...
    try {
        ...
        if (!task || !workspace_id || !payload) {
            return new Response(
                JSON.stringify({ error: 'Missing required fields: task, workspace_id, payload' }),
                { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
        }

        const systemPrompt = `
Você é um assistente de vendas inteligente e prestativo da SynapseWhats.
...
`.trim();
        ...
        if (audioUrl && !userMessage) {
            ...
            log('INFO', `[${requestId}] Audio transcribed`, { text: userMessage.substring(0, 100) });
        }
